...
from apps.robokassa.conf import USE_POST
from apps.robokassa.forms import ResultURLForm, SuccessRedirectForm, FailRedirectForm
from apps.robokassa.models import SuccessNotification
from apps.robokassa.signals import result_received, success_page_visited, fail_page_visited
...
def receive_result(request):
    ...
    if form.is_valid():
        id, sum = form.cleaned_data['InvoiceID'], form.cleaned_data['OutSum']
...
def success(request, template_name='robokassa/success.html', extra_context=None,
    ...
    if form.is_valid():
        ...
        context = {'InvoiceID': id, 'OutSum': sum, 'form': form}
